#!/usr/bin/env python3
"""
Place correct SpaceX order on Alpaca LIVE Trading
Buy 4 shares of SPCX at $136 for tomorrow
"""
...
def place_correct_spacex_order():
    """Place limit order for SPCX at $136 on live account"""
    try:
        ...
        # Order details - CORRECT TICKER
        symbol = "SPCX"  # Correct SpaceX ticker
        qty = 4
        limit_price = 136.0
        total_cost = qty * limit_price
        
        print(f"\nPlacing CORRECT LIVE order:")
        ...
        print(f"\n✅ CORRECT LIVE order placed successfully!")
        ...
        print(f"❌ Error placing correct live order: {e}")
        return None

if __name__ == "__main__":
    print("CORRECT SpaceX LIVE Order Placement - Alpaca LIVE Trading")
    print("=" * 60)
    print("⚠️  WARNING: This will place a REAL order with REAL money!")
    print("=" * 60)
    
    order = place_correct_spacex_order()
    
    if order:
        print(f"\n📊 CORRECT LIVE Order Summary:")
        print(f"- Order ID: {order.id}")
        ...
        print("\n❌ CORRECT LIVE order failed. Please check:")
        print("- Account balance")
        print("- Symbol ticker (SPCX)")
        print("- Market hours")
