...
backtest_boof21.py
==================
Boof 21.0 — Volume Cluster S/R (direct Pine port, 1-min bars)
...
TP_PCT         = 0.007   # underlying move for +40% option TP (delta ~0.5)
SL_PCT         = 0.002   # underlying move for -10% option SL
TIME_STOP_BARS = 30
...
def backtest(df: pd.DataFrame, symbol: str) -> list:
    ...
    sym_tp_pct        = cfg.get('tp_pct',          TP_PCT)
    sym_sl_pct        = cfg.get('sl_pct',          SL_PCT)
    ...
    for i in range(MIN_BARS, len(df) - TIME_STOP_BARS - 1):
        ...
        # ── Regime filter ──
        # SPY: retest only (mean-reversion, clean level bounces)
        # QQQ: retest + breakout when in trend mode (trend-following)
        if chop_mode:
            continue
        ...
        stop_price  = entry_price * (1 - sym_sl_pct) if direction == 'LONG' \
                      else entry_price * (1 + sym_sl_pct)
        tp_price    = entry_price * (1 + sym_tp_pct) if direction == 'LONG' \
                      else entry_price * (1 - sym_tp_pct)
...
def run():
    ...
    pf     = sum(wins) / abs(sum(losses)) if losses else float('inf')
    print(f"\n{'='*40}")
    print(f"COMBINED — {len(SYMBOLS)} symbols, April 2026")
