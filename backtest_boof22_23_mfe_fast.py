...
Faster version: 5m only, 3 months, key symbols
"""
import os
import pandas as pd
import numpy as np
...
SYMBOLS = ['SPY', 'QQQ', 'AAPL', 'NVDA', 'META']
...
def main():
    print("="*70)
    print("BOOF 22 & 23 — MFE Analysis (Fast Version)")
    print("3 Months | 5m only | 5 symbols")
    print("="*70)
    ...
    print("="*70)
    analyze_mfe(all_22, "Boof 22 — 5Min (Aggregate)")
    analyze_mfe(all_23, "Boof 23 — 5Min (Aggregate)")
    
    # Save
    date_str = datetime.now().strftime('%Y%m%d')
    pd.DataFrame(all_22).to_csv(f'mfe_22_5m_{date_str}.csv', index=False)
    pd.DataFrame(all_23).to_csv(f'mfe_23_5m_{date_str}.csv', index=False)
    print(f"\n[SAVED] mfe_*_{date_str}.csv")
