"""
Runner Analysis - 9:30-11:00 AM EST Only
Faster processing with focused time window
"""
import pandas as pd
import numpy as np
import glob
...
print("="*100)
print("RUNNER ANALYSIS - 9:30-11:00 AM EST ONLY")
print("="*100)
...
for symbol in symbols[:12]:  # Limit to 12 symbols max
    # Get files for this symbol
    sym_files = [f for f in files_1m if f'boof24_{symbol}_1Min_' in f]
    
    for file in sym_files[:2]:  # Limit to 2 files per symbol
        try:
            df = pd.read_csv(file)
            if len(df) == 0:
                continue
            ...
            # Filter to 9:30-11:00 AM EST only
            df['time'] = df['timestamp'].dt.time
            df['hour'] = df['timestamp'].dt.hour
            df['minute'] = df['timestamp'].dt.minute
            df['date'] = df['timestamp'].dt.date
            
            # 9:30-11:00 AM filter
            mask = ((df['hour'] == 9) & (df['minute'] >= 30)) | ((df['hour'] == 10))
            df = df[mask].copy()
            
            if len(df) < 50:  # Need enough bars
                continue
...
if all_signals:
    ...
    print(f"\n{'='*100}")
    print(f"TOTAL SIGNALS FOUND (9:30-11 AM): {len(results_df)}")
    print(f"{'='*100}")
    
    # Get runners (MFE >= 2% for short = -2%)
    ...
    if len(runners) > 0:
        ...
        for idx, row in runners.iterrows():
            ...
            print(f"  minutes_from_open: {row['minutes_from_open']}")
        ...
        # Save results
        runners.to_csv('boof30_runners_9_11am.csv', index=False)
        print(f"\n{'='*100}")
        print("SAVED: boof30_runners_9_11am.csv")
        print(f"{'='*100}")
    else:
