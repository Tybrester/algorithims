"""
Futures Cross-Reference + Boof 23 - DataBento Version 2
"""
import sys
...
print('\nFetching ES...')
es = client.timeseries.get_range(dataset='GLBX.MDP3', symbols=['ES'], stype_in='continuous', schema='ohlcv-1m', start=START, end=END)
print('Fetching NQ...')
nq = client.timeseries.get_range(dataset='GLBX.MDP3', symbols=['NQ'], stype_in='continuous', schema='ohlcv-1m', start=START, end=END)
print('Fetching MES...')
mes = client.timeseries.get_range(dataset='GLBX.MDP3', symbols=['MES'], stype_in='continuous', schema='ohlcv-1m', start=START, end=END)
print('Fetching MNQ...')
mnq = client.timeseries.get_range(dataset='GLBX.MDP3', symbols=['MNQ'], stype_in='continuous', schema='ohlcv-1m', start=START, end=END)

es_df, nq_df, mes_df, mnq_df = es.to_df(), nq.to_df(), mes.to_df(), mnq.to_df()
print(f'\nES: {len(es_df)} bars')
...
print('\nRunning backtest...')
trades = run_backtest(es_df, nq_df, mes_df, mnq_df)
...
if trades:
    ...
    print(f'Trades: {n} | WR: {wr:.1f}% | PF: {pf:.2f} | P&L: ${pnls.sum()*100:,.0f}')
    ...
    for s, v in by_sig.items():
        arr = np.array(v)
        print(f'  {s:<12} {len(arr):>4} | ${arr.sum()*100:>10,.0f}')
