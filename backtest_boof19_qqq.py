...
# =========================================================
# QQQ 19.0 - MOMENTUM ENGINE
# =========================================================

QQQ_PROFILE = {
  "mode": "momentum",
  "orbBreakoutPct": 0.012,        # 1.2%
  "liquidityGrabPct": 0.01,       # 1.0%
  "vwapDistance": 0.015,          # 1.5%
  "allowFadeBreakouts": False,
  "requireBreakAndHold": True,
  "takeProfitPct": 0.45,          # let runners go
  "stopLossPct": -0.12,
  "maxHoldMin": 15,
  "maxTradesPerDay": 5,
  "requireTrendRegime": True,
  "chopTradesAllowed": False,
...
# Dynamic TP
TP_CHOP = QQQ_PROFILE["takeProfitPct"]
TP_TREND = QQQ_PROFILE["takeProfitPct"]

# Multi-Candle Acceptance
CONTINUATION_CANDLES = 1  # Require continuation for momentum
CONTINUATION_THRESHOLD = 0.0005
...
# Distance Filters
MAX_DISTANCE_FROM_VWAP = QQQ_PROFILE["vwapDistance"]
MAX_DISTANCE_FROM_EMA = 0.015
MAX_DISTANCE_FROM_ORB = 0.03

# Reversal Logic (QQQ momentum - tighter thresholds)
FAILED_BREAKDOWN_THRESHOLD = QQQ_PROFILE["liquidityGrabPct"]
FAILED_BREAKOUT_THRESHOLD = QQQ_PROFILE["liquidityGrabPct"]
LIQUIDITY_GRAB_THRESHOLD = QQQ_PROFILE["liquidityGrabPct"]
VWAP_RECLAIM_REVERSAL = False  # Disabled for momentum

# Exit Engine
UNDERLYING_TP_PCT = QQQ_PROFILE["takeProfitPct"]
UNDERLYING_SL_PCT = QQQ_PROFILE["stopLossPct"]
VWAP_STOP_LOSS = False  # Disabled for momentum
ATR_TP_MULTIPLIER = 0.8  # Larger TP for momentum
ATR_SL_MULTIPLIER = 0.5
MAX_HOLD_MINUTES = QQQ_PROFILE["maxHoldMin"]
FAST_STOP_MINUTES = 5
...
SESSION_OPEN_TP_MULTIPLIER = 1.3
SESSION_OPEN_SL_MULTIPLIER = 0.9
SESSION_MIDDAY_TP_MULTIPLIER = 1.2
SESSION_MIDDAY_SL_MULTIPLIER = 1.0
SESSION_POWER_TP_MULTIPLIER = 1.5
SESSION_POWER_SL_MULTIPLIER = 0.8
...
def check_multi_candle_continuation(df, idx, direction, num_candles=1, threshold=0.0005):
...
def detect_breakout(df, idx, level):
    """Detect strong breakout above level"""
    if idx < 3:
        return False
    
    # Price must break and hold above level
    if df.iloc[idx]['Close'] > level * (1 + QQQ_PROFILE["orbBreakoutPct"]):
        # Check if it held for recent candles
        recent_closes = df.iloc[idx-3:idx]['Close']
        if all(close > level for close in recent_closes):
            return True
    return False

def detect_breakdown(df, idx, level):
    """Detect strong breakdown below level"""
    if idx < 3:
        return False
    
    # Price must break and hold below level
    if df.iloc[idx]['Close'] < level * (1 - QQQ_PROFILE["orbBreakoutPct"]):
        # Check if it held for recent candles
        recent_closes = df.iloc[idx-3:idx]['Close']
        if all(close < level for close in recent_closes):
...
def detect_momentum_surge(df, idx):
    """Detect momentum surge (strong directional move)"""
    if idx < 5:
        return False, 0
    
    # Calculate recent momentum
    recent_closes = df.iloc[idx-5:idx]['Close']
    momentum = (recent_closes.iloc[-1] - recent_closes.iloc[0]) / recent_closes.iloc[0]
    
    # Check for consistent directional candles
    if momentum > 0.005:  # 0.5% surge
        # Check if most candles are green
        green_candles = sum(1 for i in range(idx-5, idx) if df.iloc[i]['Close'] > df.iloc[i]['Open'])
        if green_candles >= 4:
            return True, 'buy'
    elif momentum < -0.005:  # -0.5% surge
        # Check if most candles are red
        red_candles = sum(1 for i in range(idx-5, idx) if df.iloc[i]['Close'] < df.iloc[i]['Open'])
        if red_candles >= 4:
            return True, 'sell'
...
def apply_realism(price, direction, is_entry=True):
    """Apply slippage and spread (disabled for QQQ)"""
    return price
...
def generate_signal(df, idx):
    """Generate momentum signals for QQQ"""
    if idx < 50:
    ...
    # QQQ: Only trade in trend
    if QQQ_PROFILE["requireTrendRegime"] and regime not in ['strong_trend', 'weak_trend']:
        return 'none', 0, f'Filter: {regime} regime (trend only)', regime
    ...
    has_surge, surge_direction = detect_momentum_surge(df, idx)
    if has_surge:
        if check_multi_candle_continuation(df, idx, surge_direction, CONTINUATION_CANDLES, CONTINUATION_THRESHOLD):
            return surge_direction, row['Close'], f'Momentum surge - {regime}', regime
    
    # ORB breakout with continuation (QQQ wider threshold)
    if detect_breakout(df, idx, levels['or_high']):
        if check_multi_candle_continuation(df, idx, 'buy', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD):
            return 'buy', row['Close'], f'ORB breakout - {regime}', regime
    
    if detect_breakdown(df, idx, levels['or_low']):
        if check_multi_candle_continuation(df, idx, 'sell', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD):
            return 'sell', row['Close'], f'ORB breakdown - {regime}', regime
    
    # Previous day breakout
    if detect_breakout(df, idx, levels['prev_high']):
        if check_multi_candle_continuation(df, idx, 'buy', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD):
            return 'buy', row['Close'], f'Prev day breakout - {regime}', regime
    
    if detect_breakdown(df, idx, levels['prev_low']):
        if check_multi_candle_continuation(df, idx, 'sell', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD):
            return 'sell', row['Close'], f'Prev day breakdown - {regime}', regime
...
def backtest_qqq19(symbol, start_date, end_date):
    """Run QQQ 19.0 backtest"""
    print(f"\n{'='*50}")
    print(f"BOOF 19.0 QQQ: {QQQ_PROFILE['name']}")
    print(f"{'='*50}\n")
    ...
    for idx in range(len(df)):
        ...
        if entry_time is not None:
            ...
            # VWAP stop loss (disabled for momentum)
...
if __name__ == "__main__":
    ...
    backtest_qqq19(SYMBOL, start_date, end_date)
