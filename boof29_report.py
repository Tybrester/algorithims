"""
BOOF 29 — Final Report from cached 3-year trades
Reformat results into: Year / Sector / Symbol / Monthly
"""
import pickle, os, sys
import pandas as pd
import numpy as np

# ── Load the already-computed trades from the 3-year run ─────────────
# Re-run collect quickly (data already cached + pre-grouped)
...
def collect(all_et, ema50, start, end):
    s_et = start.tz_convert("America/New_York"); e_et = end.tz_convert("America/New_York")
    ...
    for d in sorted(qqq_grp.keys()):
        ...
        for sym, grps in sym_grps.items():
            ...
            ts2=pd.Timestamp(d)
            trades.append({"date":d,"symbol":sym,"sector":SYM_TO_SECTOR.get(sym,"Other"),
                           "pnl":(xp-ep)/ep*100,"qqq_5m":round(q5*100,3),
                           "month":ts2.to_period("M"),"year":ts2.year})
            dhit=True
...
OUT = open("boof29_report.txt","w",encoding="utf-8")
_p = print
def print(*a,**k):
...
# ════════════════════════════════════════════════════════════════════
hdr("BOOF 29 -- FULL UNIVERSE 3-YEAR SUMMARY  (Long Only | 0.50-0.60% | 9:35 -> 10:20)")
print(f"  {'Period':<10} {'Trades':>8} {'T/Day':>7} {'WR%':>6} {'PF':>6} {'EV/trade':>10} {'MaxDD':>8} {'Total':>9}")
line()
for yr in ["2024","2025","2026"]:
    s=st(yt[yr]); a=ya[yr]
    if not s: continue
    tpd=s["n"]/a if a>0 else 0
    print(f"  {yr:<10} {s['n']:>8} {tpd:>7.2f} {s['wr']*100:>5.1f}% {s['pf']:>6.2f} {s['ev']:>+9.3f}% -{s['dd']:>6.2f}% {s['tot']:>+8.2f}%")
line()
s=st(all_t); tpd=s["n"]/tot_a
print(f"  {'COMBINED':<10} {s['n']:>8} {tpd:>7.2f} {s['wr']*100:>5.1f}% {s['pf']:>6.2f} {s['ev']:>+9.3f}% -{s['dd']:>6.2f}% {s['tot']:>+8.2f}%")
sep()

# ════════════════════════════════════════════════════════════════════
hdr("TOP SECTORS  (all 3 years)")
sec_rows=[]
for sec in SECTORS:
    sub=df[df["sector"]==sec]; s=st(sub)
    if not s: continue
    sec_rows.append((sec,s))
sec_rows.sort(key=lambda x:-x[1]["tot"])
print(f"  {'Sector':<22} {'Trades':>8} {'WR%':>6} {'PF':>6} {'EV/trade':>10} {'Total':>9}")
line()
for sec,s in sec_rows:
    tag=" PASS" if s["pf"]>=1.3 else (" EDGE" if s["pf"]>=1.0 else " FAIL")
    print(f"  {sec:<22} {s['n']:>8} {s['wr']*100:>5.1f}% {s['pf']:>6.2f} {s['ev']:>+9.3f}% {s['tot']:>+8.2f}%{tag}")
sep()
...
print(f"  {'Symbol':>8} {'Sector':<22} {'Trades':>7} {'WR%':>6} {'PF':>6} {'EV/trade':>10} {'Total':>9}")
line()
for _,r in sdf.head(20).iterrows():
    print(f"  {r['sym']:>8} {r['sec']:<22} {r['n']:>7} {r['wr']:>5.1f}% {r['pf']:>6.2f} {r['ev']:>+9.3f}% {r['total']:>+8.2f}%")
sep()
