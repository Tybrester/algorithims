// =========================================================
// BOOF 19.0 QQQ - 0DTE MOMENTUM SCALPING
// Trend Follower - Hold If Trend Valid
// =========================================================
...
export interface Boof19QqqResult {
...
// =========================================================
// QQQ-SPECIFIC PROFILE
// =========================================================

const QQQ_PROFILE = {
  mode: "mean_reversion",       // Changed from momentum to mean reversion
  orbBreakoutPct: 0.0028,       // 0.28%
  liquidityGrabPct: 0.0028,    // 0.28%
  vwapDistance: 0.019,          // 1.9%
...
export function generateSignalBoof19Qqq(candles: Candle[], symbol: string, tradeDirection: string = 'both'): Boof19QqqResult {
  ...
  if (symbol !== TARGET_SYMBOL) {
    return { signal: 'none', price: candles[candles.length - 1].close, reason: 'Symbol not QQQ', setupType: 'none', regime: 'chop', trendScore: 0 };
  }
  ...
  // QQQ: trend trades allowed
  if (!QQQ_PROFILE.trendTradesAllowed && (regime === 'strong_trend' || regime === 'weak_trend')) {
  ...
  // Distance Filters
  const { ok: distanceOk, reason: distanceReason } = checkDistanceFilters(candles, currentIndex, levels, QQQ_PROFILE.vwapDistance);
  if (!distanceOk) {
  ...
  if (detectFailedBreakdown(candles, currentIndex, levels.prevLow, QQQ_PROFILE.liquidityGrabPct)) {
  ...
  if (detectFailedBreakout(candles, currentIndex, levels.prevHigh, QQQ_PROFILE.liquidityGrabPct)) {
  ...
  if (detectLiquidityGrab(candles, currentIndex, levels.orHigh, QQQ_PROFILE.liquidityGrabPct)) {
    if (checkMultiCandleContinuation(candles, currentIndex, 'sell', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD)) {
      if (tradeDirection === 'both' || tradeDirection === 'short') {
  ...
  if (detectLiquidityGrab(candles, currentIndex, levels.orLow, QQQ_PROFILE.liquidityGrabPct)) {
    if (checkMultiCandleContinuation(candles, currentIndex, 'buy', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD)) {
      if (tradeDirection === 'both' || tradeDirection === 'long') {
  ...
  if (currentCandle.close > levels.orHigh * (1 + QQQ_PROFILE.orbBreakoutPct)) {
    if (checkMultiCandleContinuation(candles, currentIndex, 'buy', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD)) {
      if (tradeDirection === 'both' || tradeDirection === 'long') {
  ...
  if (currentCandle.close < levels.orLow * (1 - QQQ_PROFILE.orbBreakoutPct)) {
    if (checkMultiCandleContinuation(candles, currentIndex, 'sell', CONTINUATION_CANDLES, CONTINUATION_THRESHOLD)) {
      if (tradeDirection === 'both' || tradeDirection === 'short') {
...
export function generateExitBoof19Qqq(
  candles: Candle[],
  symbol: string,
