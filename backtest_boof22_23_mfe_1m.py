"""
BOOF 22 & 23 — Maximum Favorable Excursion (MFE) Analysis on 1m timeframe
1 year | 1m only | 9 symbols
"""
import os
import pandas as pd
import numpy as np
...
def fetch_data(symbol, start, end):
    ...
    chunk_size = 21  # Smaller chunks for 1m
    ...
    for i in range(30, len(df) - 1):
        ...
        for h in highs:
            if abs(current['close'] - h) < atr * 0.6 and current['close'] < prev['close']:
                signals.append({'direction': 'short', 'entry': df.iloc[i+1]['open'], 'idx': i + 1, 'timestamp': df.index[i+1]})
                break
        for l in lows:
            if abs(current['close'] - l) < atr * 0.6 and current['close'] > prev['close']:
                signals.append({'direction': 'long', 'entry': df.iloc[i+1]['open'], 'idx': i + 1, 'timestamp': df.index[i+1]})
                break
    return signals
...
def backtest_mfe(df, signals, max_bars=30):
    ...
    for sig in signals:
        ...
        results.append({'direction': direction, 'max_fav': max_fav, 'final_pnl': final_pnl, 'level_hit': level_hit})
...
def analyze_mfe(results, name):
    ...
    print("-" * 65)
    print(f"{'Level':>10} {'Hit':>8} {'% of Trades':>12} {'Avg Final P&L':>15} {'Win Rate':>12}")
    print("-" * 65)
    ...
    for level in levels:
        ...
        print(f"{level:>9.2f}% {count:>8} {pct:>11.1f}% {avg_final:>+14.3f}% {win_rate:>11.1f}%")
    
    print("-" * 65)
    print(f"Never hit 0.03%: {len(df_res[df_res['max_fav'] < 0.03])} trades ({len(df_res[df_res['max_fav'] < 0.03])/total*100:.1f}%)")
    print(f"Max MFE: {df_res['max_fav'].max():.3f}% | Avg MFE: {df_res['max_fav'].mean():.3f}%")
    print(f"Avg Final P&L: {df_res['final_pnl'].mean():+.3f}% | Overall Win Rate: {len(df_res[df_res['final_pnl'] > 0])/total*100:.1f}%")

def main():
    print("="*70)
    print("BOOF 22 & 23 — MFE Analysis on 1Min Timeframe")
    print("1 Year | 1m | 9 symbols")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("AGGREGATE 1MIN MFE RESULTS (All Symbols Combined)")
    print("="*70)
    ...
    pd.DataFrame(all_22).to_csv(f'mfe_boof22_1m_{date_str}.csv', index=False)
    pd.DataFrame(all_23).to_csv(f'mfe_boof23_1m_{date_str}.csv', index=False)
    print(f"\n[SAVED] mfe_boof22_1m_{date_str}.csv, mfe_boof23_1m_{date_str}.csv")
