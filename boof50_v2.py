...
API_KEY    = "PKZO6FSNSORMMBURGL66O7HR27"
API_SECRET = "4ipoweEtjBH2tJcKFf8X3bb8iW1gzvqgDtQnyjtyHjj3"
PAPER      = True
...
# ── REST polling bar feed (replaces websocket stream) ─────────────────────────

_data_client = _SHDC(API_KEY, API_SECRET)
_seen_bars   = {}  # sym -> last bar timestamp seen

def poll_bars_once():
    """Fetch latest 1-min bar for each symbol and call handle_bar if new."""
    from datetime import datetime, timedelta
    TZ = ZoneInfo("America/New_York")
    now   = datetime.now(TZ)
    start = now - timedelta(minutes=5)
    try:
        req  = _SBR(symbol_or_symbols=SYMBOLS,
                    timeframe=TimeFrame(1, TimeFrameUnit.Minute),
                    start=start, feed="iex")
        ...
        for sym in SYMBOLS:
            ...
            if _seen_bars.get(sym) == ts:
                continue
            _seen_bars[sym] = ts
            if sym in state:
                handle_bar(sym, {"o": float(bar.open), "h": float(bar.high),
                                 "l": float(bar.low),  "c": float(bar.close),
                                 "v": float(bar.volume)})
    except Exception as e:
        log.error(f"Poll bars error: {e}")
...
def main():
    ...
    log.info("REST polling mode — checking bars every 60s (IEX free feed)...")
    while True:
        poll_bars_once()
        time.sleep(60)
