...
# ── KEYS ──────────────────────────────────────────────────────────────
PAPER_KEY    = os.environ.get("ALPACA_PAPER_KEY",    "PKEOSSDLRHA5O7BRMSGEUX4Q7Y")
PAPER_SECRET = os.environ.get("ALPACA_PAPER_SECRET", "6fgvD1QALYtUnYesgtuwPW4ZZXvQqxFFzR5y7Et4b8Cf")
...
COOLDOWN_SEC = 3 * 60   # cooldown per symbol after exit (3 min)

# ── OPTIONS CONFIG ────────────────────────────────────────────────────
OPTION_TARGET  = 3.00   # target option mid price ($300/contract)
CONTRACTS      = 1
TP_MULT        = 1.50   # +50% option
SL_MULT        = 0.80   # -20% option
...
MIN_5M_BARS    = 15     # min 5-min bars required before scanning a symbol

SYMS = [
    'HOOD','MSFT','JPM','GS','AMD','ABNB','LLY','NVDA','SOFI',
    'TOST','ORCL','V','GE','SCHW','TMO','CAT','VRTX','CSCO',
]
...
def reconcile_positions():
    """On startup, re-register any open Alpaca option positions into open_positions."""
    try:
        positions    = api.list_positions()
        open_orders  = {o.symbol for o in api.list_orders(status='open') if o.side == 'sell'}
        for p in positions:
            opt_sym = p.symbol
            underlying = next((s for s in SYMS if opt_sym.startswith(s)), None)
            if not underlying:
                continue
            ...
            # find tp_id if resting limit sell exists
            tp_id = next((o.id for o in api.list_orders(status='open')
                          if o.symbol == opt_sym and o.side == 'sell' and o.type == 'limit'), None)
            with _lock:
                open_positions[underlying] = {
                    "opt_sym":   opt_sym,
                    "entry":     entry,
                    "tp":        tp_price,
                    "sl":        sl_price,
                    "qty":       qty,
                    "direction": "long" if "C" in opt_sym[len(underlying):] else "short",
                    "opened_at": datetime.datetime.now(TZ),
                    "tp_id":     tp_id,
                }
            log.info(f"RECONCILE {underlying} {opt_sym} @ {entry}  TP={tp_price}  SL={sl_price}")
    except Exception as e:
...
def select_option(sym, side, underlying_price):
    ...
    try:
        ...
        if not chain:
            # Relax spread filter to 60% and retry
            for contract in contracts:
                snap = snaps.get(contract.symbol)
                if not snap: continue
                strike = float(contract.strike_price)
        ...
        # ITM first: calls -> strike <= price, puts -> strike >= price
        if opt_type == "call":
...
def place_option_entry(sym, direction, underlying_price):
    ...
    with _lock:
        ...
        open_positions[sym] = {"reserved": True}  # reserve immediately inside lock

    contract = select_option(sym, direction, underlying_price)
    if not contract:
        log.warning(f"No suitable option for {sym} {direction}")
        with _lock: open_positions.pop(sym, None)
        return
    ...
    order_id = None
    # Attempt 1: limit at ask (should fill immediately for liquid options)
    limit_px = round(ask + 0.05, 2)  # slight above ask to guarantee fill
    ...
    log.warning(f"SKIP   {sym} {direction} — unfilled after all attempts, cancelled")
    with _lock:
    ...
    for sym in SYMS:
        ...
        try:
            trades = mod.run_boof23_strict_5sig_1exec(
                df1, sym, tp_pct=TP_PCT, sl_pct=SL_PCT, cooldown_bars=3
            )
            if not trades:
...
# ── TP/SL MONITOR ─────────────────────────────────────────────────────
def check_exits():
    """Monitor SL/TP using Alpaca real-time position current_price — fires instantly on breach."""
    ...
    try:
        ...
        log.error(f"check_exits fetch error: {e}"); return
    ...
    for sym, pos in list(open_positions.items()):
        ...
        alp = pos_by_sym.get(opt_sym)
        if alp:
            current_price = float(alp.current_price or 0)
            entry         = pos.get("entry", 0)
            if current_price > 0 and entry > 0 and opt_sym not in open_orders:
                pct = (current_price - entry) / entry * 100

                # ── Ratcheting trailing stop ──────────────────────────────
                # Update peak price seen so far
                if current_price > pos.get("peak", entry):
                    pos["peak"] = current_price
                peak = pos.get("peak", entry)
                peak_pct = (peak - entry) / entry * 100

                # Ratchet floor: activates at +25%, moves up every +5% above that
                # floor starts at +25% once activated
                if peak_pct >= 25.0:
                    # every 5% step above 25 ratchets floor up by 5%
                    steps = int((peak_pct - 25.0) / 5.0)
                    trail_floor_pct = 25.0 + steps * 5.0
                    trail_floor     = round(entry * (1 + trail_floor_pct / 100), 2)
                    if current_price <= trail_floor:
                        log.info(f"TRAIL STOP {sym} cur={current_price:.2f} <= floor={trail_floor:.2f} "
                                 f"(peak={peak:.2f} +{peak_pct:.1f}%, floor=+{trail_floor_pct:.0f}%) — CLOSING")
                        try:
                            if pos.get("tp_id"):
                            ...
                            log.error(f"Trail stop close failed {sym}: {e}")
                        to_close.append(sym); continue
                    else:
                        log.info(f"TRAIL {sym} cur={current_price:.2f} +{pct:.1f}%  peak=+{peak_pct:.1f}%  floor=+{trail_floor_pct:.0f}%")
                        continue  # don't check fixed SL/TP once trailing is active

                # ── Fixed SL (only active before +15% threshold) ─────────
                if current_price <= sl_price:
                    log.warning(f"SL HIT {sym} cur={current_price:.2f} <= sl={sl_price:.2f} — CLOSING")
                    try:
                        if pos.get("tp_id"):
                            try: api.cancel_order(pos["tp_id"])
                            except: pass
        ...
        if age_min >= 120:
            ...
            try:
                api.submit_order(pos["opt_sym"], pos.get("qty", CONTRACTS), "sell", "market", "day")
            except Exception as e:
