...
// ADX INDICATOR
// ─────────────────────────────────────────────
function computeADX(candles, len) {
    const adx = [];
    const plusDM = [];
    const minusDM = [];
    const tr = [];
...
// ─────────────────────────────────────────────
function getBoof23v2Signal(candles, symbol = 'NVDA', tpPct = 0.35, slPct = 0.15) {
    const minBars = 100;
    if (candles.length < minBars) {
        return {
            ...
            zzTrend: '', zzSwingBar: 0, zzSwingPrice: 0
        };
    }
    const adx = computeADX(candles, CHOP_CFG.ADX_LEN);
    const currentADX = adx[adx.length - 1];
    const isChop = currentADX < CHOP_CFG.ADX_CHOP_TH;
    const result = (0, boof23_1.getBoof23Signal)(candles, symbol, tpPct, slPct);
    return {
        ...result,
