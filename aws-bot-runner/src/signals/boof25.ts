...
export interface Boof25Result {
  signal:          'buy' | 'sell' | 'none';
  price:           number;
  reason:          string;
  direction:       'LONG' | 'SHORT' | 'none';
  rsi:             number;
  bbPosition:      number;  // Bollinger Band position (0-1)
  deviation:       number;  // Std dev from mean
  atr:             number;
  tpPct:           number;
  slPct:           number;
  mode:            'chop' | 'trend';
...
function computeBollingerBands(candles: Candle[], period: number, mult: number) {
  const middle: number[] = new Array(candles.length).fill(0);
...
function isBullishReversal(candles: Candle[], idx: number, lookback: number): boolean {
...
function isBearishReversal(candles: Candle[], idx: number, lookback: number): boolean {
  if (idx < lookback) return false;
  const curr = candles[idx];
  const prev = candles[idx - 1];
...
// ─────────────────────────────────────────────
export function getBoof25Signal(
  candles: Candle[],
  symbol: string,
  defaultTpPct?: number,
  defaultSlPct?: number
  ...
  if ((oversold || extremeOversold) && (atLowerBand || bullishRev)) {
    ...
    if (extremeOversold) reason += ' (extreme oversold)';
    else if (oversold) reason += ' (oversold)';
    if (atLowerBand) reason += ', at lower BB';
    if (bullishRev) reason += ', bullish reversal';
    if (rsiTurningUp) reason += ', RSI turning up';
  }
  ...
  if ((overbought || extremeOverbought) && (atUpperBand || bearishRev)) {
    ...
    if (extremeOverbought) reason += ' (extreme overbought)';
    else if (overbought) reason += ' (overbought)';
    if (atUpperBand) reason += ', at upper BB';
    if (bearishRev) reason += ', bearish reversal';
    if (rsiTurningDown) reason += ', RSI turning down';
  }
