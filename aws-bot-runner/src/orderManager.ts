import { getAlpacaCreds } from './supabaseClient';
...
function getBaseUrl(env: string, forcePaper: boolean): string {
  return (!forcePaper && env === 'live')
    ? 'https://api.alpaca.markets'
    : 'https://paper-api.alpaca.markets';
}

function formatOptionSymbol(symbol: string, expDate: string, optType: 'call' | 'put', strike: number): string {
...
async function alpacaFetch(url: string, apiKey: string, secretKey: string, method = 'GET', body?: any): Promise<any> {
...
export async function getOptionMidPrice(
  ...
  try {
    ...
    if (!ok || !data?.snapshot) return null;
...
export async function placeOrder(
  ...
  const creds = await getAlpacaCreds(userId);
  if (!creds) return { success: false, error: 'No Alpaca credentials' };
  ...
  if (!fillPrice && order.status !== 'filled' && limitPrice) {
    ...
    const { ok: pollOk, data: polled } = await alpacaFetch(
      `${baseUrl}/v2/orders/${order.id}`, api_key, secret_key
    );
