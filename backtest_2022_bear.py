"""
backtest_2022_bear.py
=====================
Test Boof 21/22/23 in 2022 bear market
Year of: Rate hikes, inflation, tech selloff, high volatility
"""
...
def main():
    print("=" * 70)
    print("2022 BEAR MARKET TEST: Boof 21 / 22 / 23")
    print("=" * 70)
    print(f"Period: {START_DATE.date()} to {END_DATE.date()}")
    print(f"Market Context: Rate hikes, inflation, tech selloff, high volatility")
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            b21 = backtest_boof21(df)
            b22 = backtest_boof22(df)
            b23 = backtest_boof23(df)
            
            all_results['Boof 21'].extend(b21)
            all_results['Boof 22'].extend(b22)
            all_results['Boof 23'].extend(b23)
            
            print(f"OK (B21={len(b21)}, B22={len(b22)}, B23={len(b23)})")
    ...
    # Summary
    print("\n" + "=" * 70)
    print("2022 BEAR MARKET RESULTS")
    print("=" * 70)
    ...
    if all_trades:
        print(f"\n{'='*70}")
        print("2022 COMBINED ALL BOTS")
        print(f"{'='*70}")
        
        combined = analyze(all_trades, "Combined 2022")
        ...
        # Compare to 2025-2026
        print(f"\n{'='*70}")
        print("2022 vs 2025-2026 COMPARISON")
        print(f"{'='*70}")
        print("2022 was bear market (tech down 30-80%)")
        print("2025-2026 was recovery/volatile bull")
        print(f"\n2022 Performance: ${combined['total_pnl']:,.2f} ({combined['trades']} trades)")
        print("If 2022 was profitable, strategy works in all regimes")
    
    # Save
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    for name, trades in all_results.items():
        if trades:
            pd.DataFrame(trades).to_csv(f'2022_bear_{name.replace(" ", "_")}_{ts}.csv', index=False)
    
    if all_trades:
        pd.DataFrame(all_trades).to_csv(f'2022_bear_combined_{ts}.csv', index=False)
