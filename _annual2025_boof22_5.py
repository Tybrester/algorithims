...
import backtest_boof22_5 as bt
...
def opt_pnl(tp, sl, tm, chop_trades=0):
    """Calculate options P&L with separate chop mode rates."""
    normal_pnl = (tp - chop_trades) * TRADE * TP_PCT + sl * TRADE * SL_PCT
    chop_pnl = chop_trades * TRADE * CHOP_TP_PCT
    tm_pnl = tm * TRADE * 0.08
    return normal_pnl + chop_pnl + tm_pnl

print('BOOF 22.5 ANNUAL BACKTEST (Chop Detection)')
...
for label, start, end, tdays in months:
    ...
    for sym in SYMBOLS:
        ...
        trades = bt.run_boof22_5(df, symbol=sym, tp_pct=STOCK_TP_PCT, sl_pct=STOCK_SL_PCT)
        ...
        # Calculate P&L with chop mode using tighter TP/SL
...
print()
print('='*75)
print('BOOF 22.5 ANNUAL SUMMARY')
print('='*75)
...
print(f"  Chop Mode:   {total_chop} ({round(100*total_chop/total_trades,1)}%)")
print(f"  Normal Mode: {total_normal} ({round(100*total_normal/total_trades,1)}%)")
