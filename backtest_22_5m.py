...
backtest_22_5m.py
=================
Boof 22 on 5-minute chart (last 15 months)
...
def backtest_boof22_5m(df):
    """Boof 22 adapted for 5-minute bars"""
    trades = []
    if len(df) < 100:
        return trades
    ...
    # Fractals on 5m — bars parameter might need adjustment
    # Use bars=2 for 5m (3 bars on 5m = 15m, too slow)
...
def main():
    ...
    if all_trades:
        stats = analyze(all_trades, "Boof 22 5m")
        ...
        print("=" * 70)
        print("1m (from prior test): 600 trades, 32.5% WR, $19.61/trade")
        print(f"5m (this test):       {stats['trades']} trades, {stats['win_rate']*100:.1f}% WR, ${stats['avg_pnl']:.2f}/trade")
        
        trade_ratio = stats['trades'] / 600
        print(f"\nTrade frequency: {trade_ratio:.1%} of 1m timeframe")
        
        if stats['win_rate'] > 0.30 and stats['profit_factor'] > 1.3:
            print("\nVERDICT: 5m timeframe WORKS — less trading, similar edge")
        else:
            print("\nVERDICT: 5m timeframe WORSE — stick to 1m")
    else:
    ...
    if all_trades:
        pd.DataFrame(all_trades).to_csv(f'boof22_5m_{ts}.csv', index=False)
        print(f"\nSaved: boof22_5m_{ts}.csv")
